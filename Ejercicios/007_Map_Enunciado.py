...
def cubo(x):
   return x*x*x
print(diccionario_original.values())
print(list(map(cubo,diccionario_original.values())))
# Ejercicio 6
...
def añadirSuf(x):
    ...
    return x+sufijo
print(list(map(añadirSuf,lista_original)))
...
def cuadrado(x):     
    return math.pow(x,2)
conjunto={map(cuadrado,lista_original)}
print(conjunto)
...
def eliminar(x):
    ...
    for i in range(len(x)):
        if x[i] not in vocales:
            resultado+=x[i]       
    return resultado
print(list(map(eliminar, lista_original)))
...
def media(x):
    notas=list(estudiantes_calificaciones.values())
    for i in range(len(notas)):
        notas[i]+=notas[i]/3
    return notas
print(list(map(media, estudiantes_calificaciones)))        
