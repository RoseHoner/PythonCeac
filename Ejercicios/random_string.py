...
lisPala = ["Mojito","PiñaColada","MoscowMule","Aperol","Tinto"]
...
numeroAadivinar=rd.randint(1,10)

check = False

while check==False:
    numeroAdivinado=int(input("Ingrese un número entre 1 y 10"))


    if numeroAadivinar==numeroAdivinado:
        print("Correcto")
        print(str(input("¿Quiere jugar de nuevo? (S/N)")))
        if str(input("¿Quiere jugar de nuevo? (S/N)"))=="S":
            check=False
        else:    
            check=True
    if numeroAadivinar>numeroAdivinado:
...
participantes=["Mia","Lua","Eva","Paco"]
elegido=""
contador=0
jugar=True
...
while jugar==True:
    for i in range(0,len(participantes)):
        elegido=rd.choice(participantes)
        print(elegido)
        contador=contador+1
        participantes.remove(elegido)
        if contador==3:
            jugar=False
            print(participantes)
            break
...
for i in range(0,len(participantes)):
    letra=rd.choice(alfabeto)
    letra1=rd.choice(alfabeto)
    letra2=rd.choice(alfabeto)
    nombreUsuario=str(rd.choice(participantes))+str(rd.randint(100,999))+letra+letra1+letra2
    print(nombreUsuario)
